...
def test_defaults_point_at_the_published_transit_feeds() -> None:
    config = load_config({})
...
def test_accepts_an_http_override() -> None:
    config = load_config({"QUEENSCOACH_ALERTS_URL": "https://example.test/a.pb"})
...
def test_rejects_a_non_http_scheme() -> None:
    with pytest.raises(ConfigError, match="http or https"):
        load_config({"QUEENSCOACH_ALERTS_URL": "file:///etc/passwd"})


@pytest.mark.parametrize(
    "env",
    [
        {"QUEENSCOACH_ALERTS_URL": "not a url"},
        {"QUEENSCOACH_ALERTS_URL": "https:///no-host"},
        {"QUEENSCOACH_REALTIME_TTL_MS": "0"},
        {"QUEENSCOACH_REALTIME_TTL_MS": "-5"},
        {"QUEENSCOACH_REALTIME_TTL_MS": "soon"},
        {"QUEENSCOACH_REALTIME_TTL_MS": "1.5"},
    ],
)
...
def test_an_empty_environment_variable_falls_back_to_the_default() -> None:
    config = load_config({"QUEENSCOACH_ALERTS_URL": "", "QUEENSCOACH_REALTIME_TTL_MS": ""})
    assert config.alerts_url.endswith("Alerts.pb")
...
def test_the_transport_can_be_set_by_environment_or_overridden() -> None:
    assert load_transport({"QUEENSCOACH_TRANSPORT": "http"}) == "http"
    assert load_transport({"QUEENSCOACH_TRANSPORT": "http"}, override="stdio") == "stdio"


def test_an_unknown_transport_is_rejected() -> None:
    with pytest.raises(ConfigError, match="unknown transport"):
        load_transport({"QUEENSCOACH_TRANSPORT": "carrier-pigeon"})
...
GOOGLE_ENV = {
    "QUEENSCOACH_GOOGLE_CLIENT_ID": "id.apps.googleusercontent.com",
    "QUEENSCOACH_GOOGLE_CLIENT_SECRET": "secret",
    "QUEENSCOACH_ALLOWED_EMAILS": "rider@example.com",
}
...
def test_command_line_arguments_win_over_the_environment() -> None:
    env = {
        **GOOGLE_ENV,
        "QUEENSCOACH_HTTP_HOST": ALL_INTERFACES,
        "QUEENSCOACH_PUBLIC_URL": "https://env.test",
    }
...
def test_the_public_url_is_reduced_to_a_bare_origin() -> None:
    ...
    config = load_http_config(
        {**GOOGLE_ENV, "QUEENSCOACH_PUBLIC_URL": "https://queenscoach.test/mcp/"}
    )
    assert config.public_url == "https://queenscoach.test"


def test_google_client_credentials_are_required() -> None:
    with pytest.raises(ConfigError, match="QUEENSCOACH_GOOGLE_CLIENT_ID is required"):
        load_http_config({"QUEENSCOACH_ALLOWED_EMAILS": "rider@example.com"})
    with pytest.raises(ConfigError, match="QUEENSCOACH_GOOGLE_CLIENT_SECRET is required"):
        load_http_config(
            {
                "QUEENSCOACH_ALLOWED_EMAILS": "rider@example.com",
                "QUEENSCOACH_GOOGLE_CLIENT_ID": "id",
            }
        )


def test_an_empty_allow_list_fails_closed() -> None:
    env = {k: v for k, v in GOOGLE_ENV.items() if k != "QUEENSCOACH_ALLOWED_EMAILS"}
...
def test_every_google_account_is_admitted_only_by_an_explicit_opt_in() -> None:
    env = {k: v for k, v in GOOGLE_ENV.items() if k != "QUEENSCOACH_ALLOWED_EMAILS"}
    config = load_http_config({**env, "QUEENSCOACH_ALLOW_ANY_GOOGLE_ACCOUNT": "true"})
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"QUEENSCOACH_ALLOWED_DOMAINS": "localhost"},
        {"QUEENSCOACH_ALLOWED_EMAILS": "rider"},
        {"QUEENSCOACH_HTTP_PORT": "70000"},
        {"QUEENSCOACH_PUBLIC_URL": "ftp://queenscoach.test"},
        {"QUEENSCOACH_ALLOW_ANY_GOOGLE_ACCOUNT": "maybe"},
        {"QUEENSCOACH_TOKEN_STORE": "redis"},
        {"QUEENSCOACH_STATELESS_HTTP": "sometimes"},
    ],
)
...
def test_allow_lists_are_case_insensitive_and_split_on_commas_or_spaces() -> None:
    config = load_http_config(
        {
            **GOOGLE_ENV,
            "QUEENSCOACH_ALLOWED_EMAILS": "Rider@Example.com, other@example.org",
            "QUEENSCOACH_ALLOWED_DOMAINS": "Charlotte.test  ridetransit.test",
        }
    )
...
def test_a_public_url_without_tls_is_refused_outside_loopback() -> None:
    with pytest.raises(ConfigError, match="must be https"):
        load_http_config({**GOOGLE_ENV, "QUEENSCOACH_PUBLIC_URL": "http://queenscoach.example.com"})
    # Loopback stays usable for trying the transport locally.
    assert (
        load_http_config(
            {**GOOGLE_ENV, "QUEENSCOACH_PUBLIC_URL": "http://127.0.0.1:8000"}
        ).public_url
        == "http://127.0.0.1:8000"
    )
...
def test_tls_is_off_unless_both_files_are_given() -> None:
    config = load_http_config({**GOOGLE_ENV, "QUEENSCOACH_PUBLIC_URL": "https://queenscoach.test"})
...
def test_tls_needs_a_certificate_and_a_key(tmp_path: Path) -> None:
    ...
    env = {
        **GOOGLE_ENV,
        "QUEENSCOACH_PUBLIC_URL": "https://queenscoach.test",
        "QUEENSCOACH_TLS_CERT": str(cert),
    }

    with pytest.raises(ConfigError, match="QUEENSCOACH_TLS_KEY must be set too"):
        load_http_config(env)

    key = tmp_path / "privkey.pem"
    key.write_text("not really a key")
    config = load_http_config({**env, "QUEENSCOACH_TLS_KEY": str(key)})
...
def test_an_unreadable_certificate_is_reported_at_startup(tmp_path: Path) -> None:
    ...
    env = {
        **GOOGLE_ENV,
        "QUEENSCOACH_PUBLIC_URL": "https://queenscoach.test",
        "QUEENSCOACH_TLS_CERT": str(missing),
    }
    ...
    with pytest.raises(ConfigError, match="cannot be read"):
        load_http_config(
            {**env, "QUEENSCOACH_TLS_CERT": str(cert), "QUEENSCOACH_TLS_KEY": str(key)}
        )
    ...
    config = load_http_config(
        {
            **GOOGLE_ENV,
            ...
            "QUEENSCOACH_FIRESTORE_DATABASE": "queenscoach",
            "QUEENSCOACH_STATELESS_HTTP": "true",
        }
    )
    assert config.oauth_store == "firestore"
    assert config.firestore_database == "queenscoach"
