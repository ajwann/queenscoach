...
def build_client(
    deps: Dependencies | None = None, identity: GoogleIdentity = ALLOWED
) -> httpx.AsyncClient:
    app = create_http_app(deps or fixture_deps(), http_config(), resolver=StubResolver(identity))
    return build_client_for(app)
...
async def test_a_google_sign_in_yields_a_token_that_opens_the_tools() -> None:
    app = create_http_app(fixture_deps(), http_config(), resolver=StubResolver(ALLOWED))
...
async def test_a_denied_google_account_never_reaches_the_token_endpoint() -> None:
    ...
    app = create_http_app(fixture_deps(), http_config(), resolver=StubResolver(denied))
