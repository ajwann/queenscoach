#!/usr/bin/env bash
#
# Deploy the QueensCoach server's HTTP transport to Google Cloud Run, in a GCP
# project of its own, and verify it from the internet.
#
#     MCP client ──https──> Cloud Run (queenscoach) ──> Firestore (sign-ins)
#
# Everything is created through the Google Cloud APIs with gcloud: the project,
# its billing link, the APIs, Firestore with TTL policies, the client secret in
# Secret Manager, least-privilege service accounts, an Artifact Registry
# repository, the image (built by Cloud Build, so no local Docker), the service,
# an optional custom domain, and a monthly budget - optionally with a kill switch
# that unlinks billing once spend nears it. The one step Google offers no API for, creating the OAuth
# client, is printed with direct links and the exact redirect URI.
#
# Re-running is safe, and is how you update: the project is found by its
# app=queenscoach label and everything that already exists is reused.
#
# Usage:
...
if [[ -n $DOMAIN ]]; then
  ...
  case "$REGION" in
    ...
    *) warn "Cloud Run may not offer domain mappings in $REGION; see deploy/GCP.md" ;;
  esac
...
if (( TEARDOWN )); then
  [[ $PROJECT_STATE == ACTIVE ]] || die "no queenscoach project to tear down"
  ...
  if [[ -n $BILLING ]]; then
    for budget in $(gcloud billing budgets list --billing-account="$BILLING" \
        --billing-project="$PROJECT" --filter="displayName=\"queenscoach: $PROJECT\"" \
...
if [[ -z $PROJECT ]]; then
  PROJECT="queenscoach-$(random_suffix)"
...
if [[ -z $PROJECT_STATE ]]; then
  gcloud projects create "$PROJECT" --name="QueensCoach" \
...
RUN_SA="$(ensure_service_account "$SERVICE-run" "queenscoach server")"
BUILD_SA="$(ensure_service_account "$SERVICE-build" "queenscoach image builds")"
...
if ! gp artifacts repositories describe "$REPOSITORY" --location="$REGION" >/dev/null 2>&1; then
  gp artifacts repositories create "$REPOSITORY" --repository-format=docker \
    --location="$REGION" --description="queenscoach server images" >/dev/null
fi
...
BUDGET_NAME="queenscoach: $PROJECT"
...
if [[ $SPEND_CAP == true ]]; then
  ...
  SPEND_SA="$(ensure_service_account "$SERVICE-spendcap" "queenscoach spend cap")"
...
for _ in $(seq 1 24); do
  if DISCOVERY="$(curl -fsS --max-time 10 \
      "$RUN_URL/.well-known/oauth-protected-resource/mcp" 2>/dev/null)"; then
    break
  fi
  DISCOVERY=""
  sleep 5
...
[[ ${RESOURCE%/} == "$PUBLIC_URL/mcp" ]] \
  || die "the server advertises ${RESOURCE:-nothing}, expected $PUBLIC_URL/mcp"
info "reachable from the internet, and the discovery document is correct"

STATUS="$(curl -s -o /dev/null -w '%{http_code}' --max-time 20 -X POST "$RUN_URL/mcp" \
